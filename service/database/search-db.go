...
func (db *appdbimpl) SearchUser(searcher User, userToSearch User) ([]User, error) {
	const query = "SELECT id_user FROM users WHERE ((id_user LIKE ?) OR (nickname LIKE ?)) AND id_user NOT IN (SELECT banner FROM banned_users WHERE banned = ?)"
	rows, err := db.c.Query(query, userToSearch.IdUser+"%", userToSearch.IdUser+"%", searcher.IdUser)
	if err != nil {
		return nil, err
	}
